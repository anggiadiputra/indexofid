...
interface TableOfContentsProps {
  content: string;
  postTitle?: string;
  postUrl?: string; // Add postUrl prop for schema
}

const TableOfContents = ({ content, postTitle, postUrl }: TableOfContentsProps) => {
  ...
  // Generate schema for TOC
  const tocSchema = useMemo(() => {
    if (!tocItems.length || !postUrl) return null;
    ...
    return {
      ...
      "mainEntity": {
        "@type": "WebPage",
        "url": postUrl,
        "hasPart": tocItems.map((item, index) => ({
          "@type": "WebPageElement",
          "name": item.text,
          "position": index + 1,
          "isPartOf": {
